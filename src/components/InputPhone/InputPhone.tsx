...
const mask = (value: string): string[] => {
  const PHONE_NUMBER_MAX_SIZE = 11;

  const digits: string[] = value
    .replace("^\\s+$", "")
    .split("")
    .filter((digit: string) => !isNaN(parseInt(digit)))
    .slice(0, PHONE_NUMBER_MAX_SIZE);

  const digitsMasked: string[] = digits.map((digit: string, idx: number) => {
    if (idx === 0) return `(${digit}`;
    if (idx === 2) return `) ${digit}`;
    if (idx === 3 && digits.length === PHONE_NUMBER_MAX_SIZE)
      return ` ${digit}`;
    if (idx === 6 && digits.length !== PHONE_NUMBER_MAX_SIZE)
      return `-${digit}`;
    if (idx === 7 && digits.length === PHONE_NUMBER_MAX_SIZE)
      return `-${digit}`;
    return digit;
  });
...
const Input: React.FunctionComponent<InputProps> = (props: InputProps) => {
  ...
  return (
    <StyledInputWrapper fluid={props.fluid}>
      ...
      <StyledInput {...props} placeholder='(__) _____-____' value={value} onChange={(evt: React.ChangeEvent<HTMLInputElement>) => {
